...
export default function PreviewFlipbook({ title, cover, pages = [], pdfUrl, previewUrl, language = "fr", onClose, onPurchase }: FlipbookProps) {
  ...
  const [protectedImageTimedOut, setProtectedImageTimedOut] = useState(false);
  ...
  useEffect(() => {
    ...
    setProtectedImageTimedOut(false);
    if (!previewUrl) return;
    const timeout = window.setTimeout(() => setProtectedImageTimedOut(true), 12000);
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#1b1c1c]/90 p-3 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="flipbook-title">
    <div className="flex max-h-[98vh] w-full max-w-6xl flex-col overflow-hidden rounded-none sm:rounded-2xl bg-[#f7f3f1] shadow-2xl">
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto bg-[#2b2525] p-0 sm:p-2 md:p-5"><div className={`relative flex w-full items-center justify-center ${showSpread ? "max-w-[1100px] gap-0" : "max-w-[560px]"} ${turning ? `flipbook-turn-${turnDirection}` : ""}`}>
        <div className={`relative aspect-[3/4] w-full overflow-hidden bg-white shadow-2xl ${showSpread ? "rounded-l-md" : "rounded-md"}`}><canvas ref={canvasLeftRef} className={`${usingPdf && !isBlocked(page) ? "block" : "hidden"} h-full w-full object-contain`} aria-label={`${title}, page ${page}`} />{usingProtectedPreview && !protectedImageLoaded && !isBlocked(page) && <img src={cover} alt={`${title}, couverture`} className="absolute inset-0 h-full w-full object-contain" />}{!usingPdf && !usingProtectedPreview && !isBlocked(page) && <img src={currentImage} alt={`${title}, page ${page}`} className="h-full w-full object-contain" />}{usingProtectedPreview && !isBlocked(page) && <img src={previewImage(page)} alt={`${title}, page ${page}`} onLoad={() => setProtectedImageLoaded(true)} onError={() => setProtectedImageLoaded(false)} className={`h-full w-full object-contain ${protectedImageLoaded ? "block" : "absolute inset-0 opacity-0"}`} />}{!usingPdf && !pdfLoading && !hasPreview && !isBlocked(page) && <div className="absolute inset-0 grid place-items-center p-6 text-center text-sm text-[#746665]">Aucun aperçu n’est encore disponible.</div>}{usingProtectedPreview && !protectedImageLoaded && !isBlocked(page) && <div className="absolute inset-x-0 bottom-4 mx-auto w-fit rounded-full bg-black/60 px-3 py-1 text-[11px] font-semibold text-white">{protectedImageTimedOut ? "Aperçu lent — la couverture reste disponible" : "Chargement de la page…"}</div>}{isBlocked(page) && <LockedPage onPurchase={onPurchase} />}</div>

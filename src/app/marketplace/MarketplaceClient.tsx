...
function ProductCard({ product }: { product: MarketplaceProduct & { isMagazine?: boolean; magazineId?: string; magazineNumero?: number } }) {
  ...
  const openProduct = () => { window.location.assign(product.isMagazine && product.magazineId ? `/kiosque/${product.magazineId}` : `/marketplace/produits/${product.id}`); };
  return <article role="link" tabIndex={0} onClick={openProduct} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openProduct(); } }} className="group cursor-pointer overflow-hidden rounded-[22px] border border-[#eadfce] bg-white shadow-[0_12px_34px_rgba(74,48,18,0.06)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(74,48,18,0.13)]">

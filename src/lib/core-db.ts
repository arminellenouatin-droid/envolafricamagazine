...
function mapArticle(row: Record<string, unknown>): Article {
  return {
    ...
    translations: row.translations && typeof row.translations === "object" ? row.translations as Article["translations"] : {},
    hasAudio: Boolean(row.has_audio ?? false) || Boolean(row.audio_by_language && typeof row.audio_by_language === "object" && Object.keys(row.audio_by_language as object).length),
    audioUrl: typeof row.audio_url === "string" ? row.audio_url : undefined,
    audioByLanguage: row.audio_by_language && typeof row.audio_by_language === "object" ? row.audio_by_language as Article["audioByLanguage"] : {},

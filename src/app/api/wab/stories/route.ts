...
export async function POST(request: NextRequest) {
  ...
  const durationSeconds = typeof body.durationSeconds === "number" ? Math.min(30, Math.round(body.durationSeconds)) : undefined;
  const now = new Date();
  ...
  if (supabase) {
    const premium = await isWabPremiumUser(user.id);
    const { data, error } = await supabase.from("wab_stories").insert({
      user_id: user.id,
    ...
    }).select("id,user_id,author,media_url,mime_type,caption,views,likes").single();

...
export default function ParametresPage(){
  ...
  async function savePreferences() {
    setPreferencesLoading(true); setPreferencesMessage("");
    try {
      const response = await fetch("/api/profile/preferences", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lang, currency }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Impossible d’enregistrer les préférences.");
      setUser((current: any) => current ? { ...current, lang, currency } : current);
      setPreferencesMessage("Préférences enregistrées. Elles seront utilisées pour les articles et leurs audios.");
    } catch (error) { setPreferencesMessage(error instanceof Error ? error.message : "Impossible d’enregistrer les préférences."); }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[20px] border p-6 space-y-6">
        ...
        <div><div className="font-bold text-[14px]">Langue & devise</div><p className="mt-1 text-[12px] leading-5 text-zinc-500">Votre langue préférée sera utilisée par défaut pour les versions traduites et les audios des articles Magazine.</p><div className="mt-3 flex flex-wrap gap-2"><select value={lang} onChange={e=>setLang(e.target.value)} aria-label="Langue préférée" className="h-11 rounded-full border bg-zinc-50 px-5 text-[13px]"><option value="fr">Français</option><option value="en">English</option><option value="es">Español</option><option value="sw">Swahili</option><option value="fon">Fongbé</option><option value="wo">Wolof</option></select><select value={currency} onChange={e=>setCurrency(e.target.value)} aria-label="Devise préférée" className="h-11 rounded-full border bg-zinc-50 px-5 text-[13px]"><option value="XOF">F CFA</option><option value="EUR">EUR €</option><option value="USD">USD $</option><option value="NGN">NGN ₦</option><option value="GHS">GHS ₵</option><option value="KES">KES KSh</option></select><button type="button" onClick={savePreferences} disabled={preferencesLoading} className="h-11 rounded-full bg-[#0A1931] px-5 text-[12px] font-bold text-white disabled:opacity-50">{preferencesLoading ? "Enregistrement…" : "Enregistrer la préférence"}</button></div>{preferencesMessage && <p className={`mt-2 text-[12px] ${preferencesMessage.startsWith("Préférences") ? "text-emerald-700" : "text-red-700"}`}>{preferencesMessage}</p>}</div>

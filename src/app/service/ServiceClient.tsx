        ...
        {sent ? (
          ...
          <form onSubmit={submit} className="mt-8 bg-white rounded-[24px] border border-zinc-100 p-6 md:p-8 space-y-4">
            <div className="grid md:grid-cols-2 gap-3">
              <div><label className="text-[11px] font-bold uppercase tracking-wide text-zinc-600">Nom complet</label><input required value={form.nom} onChange={e=>setForm({...form, nom:e.target.value})} placeholder="Ex : Koffi Jean-Baptiste" className="mt-1.5 w-full h-11 rounded-full border bg-zinc-50 px-5 text-[13px]" /></div>
              <div><label className="text-[11px] font-bold uppercase tracking-wide text-zinc-600">Email professionnel</label><input required type="email" value={form.email} onChange={e=>setForm({...form, email:e.target.value})} placeholder="votre-email@entreprise.com" className="mt-1.5 w-full h-11 rounded-full border bg-zinc-50 px-5 text-[13px]" /></div>
            </div>
            ...
            <div className="text-[11px] text-zinc-500 text-center">Réponse garantie sous 24h ouvrées • Support dédié pour abonnés et partenaires</div>
          </form>
        )}

        <div className="mt-10 grid md:grid-cols-3 gap-3">
          <div className="rounded-[14px] bg-white border p-4"><div className="font-bold text-[13px]">💼 Emploi</div><div className="text-[12px] text-zinc-600 mt-1">Recrutements & Talents panafricains</div></div>
          <div className="rounded-[14px] bg-white border p-4"><div className="font-bold text-[13px]">🛒 Marketplace</div><div className="text-[12px] text-zinc-600 mt-1">Fournisseurs & Produits vérifiés</div></div>
          <div className="rounded-[14px] bg-white border p-4"><div className="font-bold text-[13px]">💰 Financement</div><div className="text-[12px] text-zinc-600 mt-1">Opportunités d'investissement</div></div>
        </div>
      </div>
    </div>
  );

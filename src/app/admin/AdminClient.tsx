...
export default function AdminDashboardClient({ user, stats, db }: { user: any, stats: any, db: any }) {
  ...
  const handleCreateArticle = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload: any = {
        title: form.get("title"), summary: form.get("summary"), content: form.get("content"), category: categories.find((item) => item.id === form.get("categoryId"))?.label || "Economie", categoryId: form.get("categoryId") || null, image: form.get("image") || articleImage,
      ...
      <div className="max-w-[1440px] mx-auto px-6 xl:px-8 pt-6">
        ...
        {activeTab==="articles" && (
          <div className="bg-white rounded-[18px] border p-6">
                ...
                <form onSubmit={handleCreateArticle} className="bg-white rounded-[20px] p-6 w-full max-w-[760px] max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="mt-4 grid gap-3">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <div><label className="mb-1 block text-[11px] font-bold text-zinc-600">Catégorie</label><div className="flex gap-2"><select name="categoryId" value={selectedCategoryId || editingArticle?.categoryId || editingArticle?.category_id || categories.find((item) => item.label.toLowerCase() === String(editingArticle?.category || "").toLowerCase())?.id || ""} onChange={(event) => setSelectedCategoryId(event.target.value)} className="h-11 min-w-0 flex-1 rounded-full border bg-zinc-50 px-4 text-[13px]" required><option value="">Sélectionner une catégorie</option>{categories.filter((item) => item.is_active !== false).map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><button type="button" onClick={() => setShowCategoryModal(true)} className="h-11 shrink-0 rounded-full border border-[#9e001f] px-3 text-[11px] font-bold text-[#9e001f]">Créer</button></div></div>
            ...
            {showAuthorModal && <div className="fixed inset-0 z-[60] grid place-items-center bg-black/50 p-4"><form onSubmit={async (event) => { event.preventDefault(); try { const form = new FormData(event.currentTarget); const createdAuthor = await createEditorialAuthor({ name: form.get("name"), bio: form.get("bio"), roleLabel: form.get("roleLabel"), photoUrl: authorPhoto }); setSelectedAuthorId(createdAuthor.id); setMessage("Rédacteur créé"); setShowAuthorModal(false); setAuthorPhoto(""); } catch (error) { setMessage(`Erreur : ${error instanceof Error ? error.message : "réessayez"}`); } }} className="w-full max-w-[520px] rounded-[20px] bg-white p-6 shadow-2xl"><div className="flex items-center justify-between"><div><h3 className="font-bold text-[#0A1931]">Créer un rédacteur</h3><p className="mt-1 text-[11px] text-zinc-500">Le portrait sera affiché sur la page publique de ses articles.</p></div><button type="button" onClick={() => setShowAuthorModal(false)} className="text-xl text-zinc-400">×</button></div><div className="mt-5 grid gap-3"><input name="name" required placeholder="Nom complet" className="h-11 rounded-full border bg-zinc-50 px-4 text-sm"/><input name="roleLabel" placeholder="Fonction, ex. Analyste économique" className="h-11 rounded-full border bg-zinc-50 px-4 text-sm"/><textarea name="bio" required rows={4} placeholder="Description du rédacteur" className="rounded-[14px] border bg-zinc-50 p-4 text-sm"/><div className="flex gap-2"><label className="inline-flex h-10 cursor-pointer items-center rounded-full bg-[#0A1931] px-4 text-xs font-bold text-white">{uploadingAuthorPhoto ? "Upload…" : "Choisir le portrait"}<input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" disabled={uploadingAuthorPhoto} onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleAuthorPhotoUpload(file); }}/></label><input name="photoUrl" value={authorPhoto} onChange={(event) => setAuthorPhoto(event.target.value)} placeholder="URL du portrait" className="h-10 min-w-0 flex-1 rounded-full border px-3 text-xs"/></div>{authorPhoto && <img src={authorPhoto} alt="Aperçu portrait" className="h-28 w-24 rounded-xl object-cover"/>}</div><button type="submit" disabled={uploadingAuthorPhoto} className="mt-5 h-10 w-full rounded-full bg-[#9e001f] text-sm font-bold text-white">Créer le rédacteur</button></form></div>}
            {showCategoryModal && <div className="fixed inset-0 z-[60] grid place-items-center bg-black/50 p-4"><form onSubmit={async (event) => { event.preventDefault(); try { const form = new FormData(event.currentTarget); const createdCategory = await createEditorialCategory({ label: form.get("label"), colorHex: form.get("colorHex") }); setSelectedCategoryId(createdCategory.id); setMessage("Catégorie créée"); setShowCategoryModal(false); } catch (error) { setMessage(`Erreur : ${error instanceof Error ? error.message : "réessayez"}`); } }} className="w-full max-w-[420px] rounded-[20px] bg-white p-6 shadow-2xl"><div className="flex items-center justify-between"><h3 className="font-bold text-[#0A1931]">Créer une catégorie</h3><button type="button" onClick={() => setShowCategoryModal(false)} className="text-xl text-zinc-400">×</button></div><div className="mt-5 grid gap-3"><input name="label" required placeholder="Nom de la catégorie" className="h-11 rounded-full border bg-zinc-50 px-4 text-sm"/><input name="colorHex" defaultValue="#9e001f" placeholder="Couleur hexadécimale" className="h-11 rounded-full border bg-zinc-50 px-4 text-sm"/></div><button type="submit" className="mt-5 h-10 w-full rounded-full bg-[#9e001f] text-sm font-bold text-white">Créer la catégorie</button></form></div>}
          </div>
        )}

        {activeTab==="redacteurs" && (
          <div className="rounded-[18px] border bg-white p-6"><div className="flex items-center justify-between"><div><h3 className="text-lg font-bold text-[#0A1931]">Rédacteurs</h3><p className="mt-1 text-xs text-zinc-500">Portraits, biographies et fonctions utilisés par les pages articles.</p></div><button onClick={() => { setActiveTab("articles"); setShowAuthorModal(true); }} className="h-10 rounded-full bg-[#0A1931] px-4 text-xs font-bold text-white">+ Nouveau rédacteur</button></div><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{authors.map((author) => <div key={author.id} className="flex gap-3 rounded-[14px] border p-4"><img src={author.photo_url || "/logo-blanc-footer.png"} alt="" className="h-16 w-14 rounded-xl bg-[#0A1931] object-cover"/><div className="min-w-0"><div className="font-bold text-[#0A1931]">{author.name}</div><div className="text-[11px] text-[#9e001f]">{author.role_label || "Rédacteur"}</div><p className="mt-2 line-clamp-3 text-xs text-zinc-600">{author.bio || "Aucune description"}</p></div></div>)}</div></div>
        )}

        {activeTab==="categories" && (
          <div className="rounded-[18px] border bg-white p-6"><div className="flex items-center justify-between"><div><h3 className="text-lg font-bold text-[#0A1931]">Catégories éditoriales</h3><p className="mt-1 text-xs text-zinc-500">Les catégories sont sélectionnées dans le formulaire article.</p></div><button onClick={() => { setActiveTab("articles"); setShowCategoryModal(true); }} className="h-10 rounded-full bg-[#9e001f] px-4 text-xs font-bold text-white">+ Nouvelle catégorie</button></div><div className="mt-6 flex flex-wrap gap-3">{categories.map((category) => <div key={category.id} className="rounded-full border px-4 py-2 text-xs font-bold" style={{ borderColor: category.color_hex || "#9e001f", color: category.color_hex || "#9e001f" }}>{category.label}</div>)}</div></div>
        )}

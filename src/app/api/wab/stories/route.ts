...
export async function GET() {
  ...
  if (supabase) {
    const { data, error } = await supabase.from("wab_stories").select("id,user_id,author,media_url,mime_type,caption,views,likes,created_at,expires_at").eq("moderation_status", "published").gt("expires_at", new Date().toISOString()).order("created_at", { ascending: false });
    if (error) return NextResponse.json({ error: "Stories indisponibles." }, { status: 503 });
    return NextResponse.json({ stories: (data ?? []).map((story) => ({ id: story.id, author: story.author, authorUserId: story.user_id, mediaUrl: story.media_url, mimeType: story.mime_type, caption: story.caption, views: story.views, likes: story.likes })) });
  }
  const db = readWabDB();
...
export async function POST(request: NextRequest) {
  ...
  const supabase = getSupabaseAdmin();
  if (supabase) {
    const { data, error } = await supabase.from("wab_stories").insert({ user_id: user.id, author: `${user.prenom} ${user.nom}`, media_url: body.mediaUrl, mime_type: typeof body.mimeType === "string" ? body.mimeType : "image/jpeg", caption: typeof body.caption === "string" ? body.caption.slice(0, 300) : "", created_at: now.toISOString(), expires_at: expiresAt, moderation_status: "published" }).select("id,user_id,author,media_url,mime_type,caption,views,likes").single();
    if (error || !data) return NextResponse.json({ error: "Impossible d’enregistrer la Story dans Supabase." }, { status: 503 });
    return NextResponse.json({ story: { id: data.id, author: data.author, authorUserId: data.user_id, mediaUrl: data.media_url, mimeType: data.mime_type, caption: data.caption, views: data.views, likes: data.likes } }, { status: 201 });
  }

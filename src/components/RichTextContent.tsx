import { fixMojibake, isRichText, plainTextToRichHtml, sanitizeRichText } from "@/lib/rich-text";

export default function RichTextContent({ value, className = "" }: { value: string; className?: string }) {
  const healed = fixMojibake(value || "");
  const html = isRichText(healed) ? sanitizeRichText(healed) : plainTextToRichHtml(healed);

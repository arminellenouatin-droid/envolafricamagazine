...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (client) {
      ...
      const republication = article?.isPublished ? await publishArticleToWab(article, user!.id) : { published: false, reason: "article_draft" };
    ...
    writeDB(db!);
    const republication = article.isPublished ? await publishArticleToWab(article, user!.id) : { published: false, reason: "article_draft" };
    const notifications = isPublishing ? await notifyPushSubscribers({ platform: "magazine", type: "new_article", title: "ENVOL AFRICA", body: article.title, link: `/article/${article.slug}`, image: article.image, entityType: "article", entityId: article.id, dedupePrefix: `article:${article.id}` }) : { count: 0 };
    return NextResponse.json({ success: true, article, republication, notifications });
  } catch (e) {
    console.error(e);

...
export default function PostActions({ postId, initialLikes, initialComments, initialShares, views, canBoost = false, onComment }: { postId: string; initialLikes: number; initialComments: number; initialShares: number; views: number; canBoost?: boolean; onComment?: () => void }) {
  ...
  async function shareTo(destination: "story" | "feed" | "friend" | "whatsapp" | "copy") {
    const canonicalBase = process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXT_PUBLIC_BASE_URL || "https://www.envolafrica.site";
    ...
    if (destination === "whatsapp") {
      ...
      try { await navigator.clipboard.writeText(url); setNotice("Lien de la publication copié."); } catch { window.prompt("Copiez le lien de la publication", url); }
    }
    if (destination === "story") {
      ...
      setNotice("Lien de la publication partagé.");
  ...
  return <>
    ...
    {shareOpen && <div className="rounded-2xl border border-[#d1e9e6] bg-white p-3 shadow-sm" role="dialog" aria-label="Choisir la destination du partage"><p className="text-xs font-bold text-[#082843]">Partager cette publication</p><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-5"><button type="button" onClick={() => void shareTo("whatsapp")} className="rounded-xl bg-[#dff7e7] px-2 py-3 text-xs font-bold text-[#137333]"><span className="material-symbols-outlined block text-lg">chat</span>WhatsApp</button><button type="button" onClick={() => void shareTo("copy")} className="rounded-xl bg-[#e9f7f5] px-2 py-3 text-xs font-bold text-[#006874]"><span className="material-symbols-outlined block text-lg">link</span>Copier le lien</button><button type="button" onClick={() => void shareTo("story")} className="rounded-xl bg-[#e9f7f5] px-2 py-3 text-xs font-bold text-[#006874]"><span className="material-symbols-outlined block text-lg">auto_stories</span>Story</button><button type="button" onClick={() => void shareTo("feed")} className="rounded-xl bg-[#e9f7f5] px-2 py-3 text-xs font-bold text-[#006874]"><span className="material-symbols-outlined block text-lg">dynamic_feed</span>Fil WAB</button><button type="button" onClick={() => void shareTo("friend")} className="rounded-xl bg-[#e9f7f5] px-2 py-3 text-xs font-bold text-[#006874]"><span className="material-symbols-outlined block text-lg">person_add</span>Un ami</button></div><button type="button" onClick={() => setShareOpen(false)} className="mt-2 text-xs font-semibold text-slate-500">Annuler</button></div>}

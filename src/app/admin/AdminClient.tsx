    ...
    <div className="bg-[#F8FAFC] min-h-screen pb-20">
      ...
      <div className="max-w-[1440px] mx-auto px-6 xl:px-8 pt-6">
        ...
        {activeTab==="categories" && (
          <div className="space-y-5"><div className="rounded-[18px] border bg-white p-6"><div className="flex items-center justify-between"><div><h3 className="text-lg font-bold text-[#0A1931]">Catégories d’articles</h3><p className="mt-1 text-xs text-zinc-500">Référentiel réservé aux articles éditoriaux.</p></div><button onClick={() => { setActiveTab("articles"); setShowCategoryModal(true); }} className="h-10 rounded-full bg-[#9e001f] px-4 text-xs font-bold text-white">+ Nouvelle catégorie article</button></div><div className="mt-6 flex flex-wrap gap-3">{categories.map((category) => <div key={category.id} className="rounded-full border px-4 py-2 text-xs font-bold" style={{ borderColor: category.color_hex || "#9e001f", color: category.color_hex || "#9e001f" }}>{category.label}</div>)}</div></div><div className="rounded-[18px] border bg-white p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-lg font-bold text-[#0A1931]">Catégories Magazine</h3><p className="mt-1 text-xs text-zinc-500">Référentiel séparé pour les numéros du Magazine.</p></div><div className="flex gap-2"><input value={newMagazineCategory} onChange={(event) => setNewMagazineCategory(event.target.value)} placeholder="Nouvelle catégorie Magazine" className="h-10 rounded-full border bg-zinc-50 px-4 text-xs"/><button type="button" onClick={() => void createMagazineCategory()} className="h-10 rounded-full bg-[#0A1931] px-4 text-xs font-bold text-white">+ Ajouter</button></div></div><div className="mt-6 flex flex-wrap gap-3">{magazineCategories.map((category) => <div key={category.id} className="flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold" style={{ borderColor: category.color_hex || "#9e001f", color: category.color_hex || "#9e001f" }}><span>{category.label}</span><button type="button" onClick={() => void deleteMagazineCategory(category)} aria-label={`Désactiver ${category.label}`} className="text-zinc-400 hover:text-red-700">×</button></div>)}</div></div></div>
        )}

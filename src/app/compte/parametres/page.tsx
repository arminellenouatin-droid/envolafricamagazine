...
export default function ParametresPage(){
  ...
  async function uploadAvatar(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setAvatar(data.avatar); setUser((current: any) => current ? { ...current, avatar: data.avatar } : current); setAvatarMessage("Photo de profil enregistrée.");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[20px] border p-6 space-y-6">
        <div><div className="font-bold text-[14px]">Photo de profil</div><div className="mt-3 flex flex-wrap items-center gap-4"><div className="grid h-20 w-20 place-items-center overflow-hidden rounded-full bg-[#0A1931] text-xl font-bold text-white">{avatar ? <img src={avatar} alt="Votre photo de profil" className="h-full w-full object-cover" /> : `${user?.prenom?.[0] || ""}${user?.nom?.[0] || ""}`}</div><div><label className="inline-flex cursor-pointer items-center rounded-full bg-[#0A1931] px-4 py-2 text-[12px] font-bold text-white">{avatarLoading ? "Enregistrement..." : "Ajouter une photo"}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={uploadAvatar} disabled={avatarLoading} className="sr-only" /></label><p className="mt-2 text-[11px] text-zinc-500">JPG, PNG ou WebP · 5 Mo maximum.</p>{avatarMessage && <p className="mt-1 text-[12px] text-emerald-700">{avatarMessage}</p>}</div></div></div>

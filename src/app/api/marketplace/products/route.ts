...
export async function POST(request: NextRequest) {
  ...
  if (body.enableAffiliation) {
    const rate = Math.min(0.5, Math.max(0.01, Number(body.affiliationRate) || 0.10));
    try {
      const { enableProductAffiliation } = await import("@/lib/affiliation/marketplace");
      await enableProductAffiliation({ productId: data.id, vendorId: user.id, commissionRate: rate });
    } catch (affErr) {
      console.warn("Could not enable product affiliation:", affErr);
    }
  }

  return NextResponse.json({ product: data, notice: initialStatus === "published" ? "Votre produit est maintenant en ligne dans le catalogue." : "Le produit est enregistré en brouillon." }, { status: 201 });
}

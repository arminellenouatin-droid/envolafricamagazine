      ...
      <div ref={feedTopRef} className="mx-auto max-w-[1240px] px-3 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-6 lg:grid lg:grid-cols-[250px_1fr] xl:grid-cols-[270px_1fr_310px]">
          ...
          <aside className="flex flex-col gap-4">
              ...
              <div className="relative px-4 pb-4">
                <div className="-mt-9 mb-3 flex items-center justify-between">
                  <div className="h-16 w-16 overflow-hidden rounded-full border-2 border-white bg-[#d7e5e3] shadow-md">
                    {currentUser?.avatar ? (
                      <img src={currentUser.avatar} alt={userDisplayName} className="h-full w-full object-cover" />
                    ) : (
                      <span className="grid h-full w-full place-items-center text-lg font-bold text-[#006874]">{userInitials}</span>
                    )}
                  </div>
                  {isBusiness ? (
                    ...
                    <button
                      type="button"
                      onClick={() => setUpgradeRequired("large")}
                ...
                <a href="/wab/profil" className="group block">
                  ...
                  <p className="truncate text-xs text-[#5f6368]">
                    {currentUser ? "Membre Envol Africa · Réseau WAB" : "Visiteur WAB"}
                  </p>
                </a>

                {/* LinkedIn-style profile metrics */}
                <div className="mt-4 border-t border-[#edf2f4] pt-3 text-xs">
                  <div className="flex items-center justify-between py-1 text-[#5f6368] hover:text-[#006874] cursor-pointer">
                ...
                {!isBusiness && (
                  <div className="mt-3 border-t border-[#edf2f4] pt-3">
                    <button
                      ...
                      className="w-full text-left rounded-lg bg-amber-50/80 p-2.5 transition hover:bg-amber-100/80 border border-amber-200/60"
                    >
            ...
            {/* Navigation Card */}
            <div className="sticky top-20 overflow-hidden rounded-2xl border border-[#d8e2e6] bg-white p-3 shadow-sm">
              <p className="px-3 pb-2 pt-1 text-[11px] font-bold uppercase tracking-wider text-[#82888e]">
                Navigation & Services
              </p>
              <nav className="flex flex-col gap-0.5 text-xs font-semibold" aria-label="Menu principal WAB">
                {leftNavLinks.map((item) => (
                  ...
                  >
                    <span className={`material-symbols-outlined text-[20px] ${item.active ? "text-[#006874]" : "text-[#5f6368]"}`}>
                      {item.icon}
                    </span>

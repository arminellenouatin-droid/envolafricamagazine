...
if (fs.existsSync(DB_FILE)) {
  try { db = JSON.parse(fs.readFileSync(DB_FILE,'utf-8')); } catch { db = null; }
}
if (!db) {
  db = { users:[], articles:[], magazines:[], orders:[], affiliateEarnings:[], donations:[], comments:[], settings:{ homeSections:{}, ads:[] } };
}
...
if (!admin) {
  admin = {
    ...
    country: "BJ",
    affiliateCode: "QUE"+Math.floor(1000+Math.random()*9000),
    favorites: [],
    downloads: [],
    phone: "+22900000000",
  };
  ...
  admin.nom = "DAVAKAN";
  admin.prenom = "Quentin";
  console.log("Admin updated");
}

fs.writeFileSync(DB_FILE, JSON.stringify(db, null, 2));
console.log("DB seeded at", DB_FILE);
console.log("Users:", db.users.length, "Articles:", db.articles.length, "Magazines:", db.magazines.length);
console.log("Admin login:", adminEmail, "/", plain);

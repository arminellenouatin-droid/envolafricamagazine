...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (supabase) {
    const { data: page } = await supabase.from("wab_pages").select("id,owner_user_id,name,slug,logo_url,description,status,created_at").eq("id", id).eq("status", "active").maybeSingle();
    ...
    return NextResponse.json({ page: { id: page.id, ownerUserId: page.owner_user_id, name: page.name, slug: page.slug, logoUrl: page.logo_url, description: page.description, postCount: count ?? 0 } });
  }
  const db = readWabDB();

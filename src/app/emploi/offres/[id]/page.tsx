...
export default async function JobOfferPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const jobSchema = getJobPostingSchema({
    ...
    companyName: unlocked ? offer.companyName : "Entreprise Partenaire Envol Africa",
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: "Offres", url: "/emploi/offres" },
    { name: offer.title, url: `/emploi/offres/${encodeURIComponent(offer.id)}` },
  ]);
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#f7f8fa] py-10">
        <OfferViewTracker offerId={offer.id} />
    ...
    <article className="mt-5 rounded-3xl bg-white p-6 shadow-sm sm:p-10">
      ...
      {unlocked ? <section className="mt-8 rounded-2xl border border-emerald-200 bg-emerald-50 p-6"><p className="font-bold text-emerald-900">Informations de l’employeur décryptées</p><p className="mt-2 text-sm text-emerald-800">{offer.companyName} · {offer.address || `${offer.city}, ${offer.country}`}</p><p className="text-sm text-emerald-800">{offer.contactEmail}{offer.contactPhone ? ` · ${offer.contactPhone}` : ""}</p><ApplyButton offerId={offer.id} /></section> : <section className="mt-8 rounded-2xl bg-[#071b36] p-6 text-white"><p className="text-xs font-bold uppercase tracking-wider text-[#8ee0c0]">Informations protégées</p><h2 className="mt-2 font-display text-2xl font-extrabold">Postulez en toute sécurité</h2><p className="mt-3 max-w-xl text-sm leading-6 text-slate-300">Le nom, les coordonnées et le moyen de soumission de cette entreprise sont volontairement chiffrés. Décryptez cette offre pour <VisitorPrice amountInXof={200} /> ou activez un abonnement candidat.</p><div className="mt-5 flex flex-wrap gap-3"><Link href={`/emploi/offres/${offer.id}/decrypter`} className="inline-flex items-center gap-1.5 rounded-xl bg-[#c91f3b] px-5 py-3 text-sm font-bold"><span>Décrypter pour</span><VisitorPrice amountInXof={200} /></Link><Link href="/emploi/abonnements" className="rounded-xl border border-white/30 px-5 py-3 text-sm font-bold">Voir les abonnements</Link></div></section>}
    </article>
        </div>
      </main>
    </>

...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (supabase) {
    ...
    return NextResponse.json({ group: { id: group.id, ownerUserId: group.owner_user_id, name: group.name, slug: group.slug, description: group.description, logoUrl: group.logo_url, avatarUrl: group.avatar_url ?? group.logo_url, coverUrl: group.cover_url, privacy: group.privacy, status: group.status, memberCount: count ?? 0 }, membership });
  }
  const db = readWabDB();

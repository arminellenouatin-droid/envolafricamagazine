...
export default function PreviewFlipbook({ title, cover, pages = [], pdfUrl, language = "fr", onClose, onPurchase }: PreviewFlipbookProps) {
  ...
  useEffect(() => {
    ...
    }).catch(() => setPdfError("Le moteur PDF n’est pas disponible dans cet aperçu."));
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#1b1c1c]/90 p-3 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="flipbook-title">
    <div className="flex max-h-[96vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-[#f7f3f1] shadow-2xl">
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto bg-[#2b2525] p-3 sm:p-8"><div className="relative flex aspect-[3/4] w-full max-w-[520px] items-center justify-center overflow-hidden rounded-md bg-white shadow-2xl"><canvas ref={canvasRef} className={`${usingPdf && !isBlocked ? "block" : "hidden"} h-full w-full object-contain`} aria-label={`${title}, page ${page}`} /><img src={usingPdf ? undefined : currentImage} alt={`${title}, page ${page}`} className={`${usingPdf ? "hidden" : "block"} h-full w-full object-cover ${isBlocked ? "opacity-0" : ""}`} />{!hasPreview && !isBlocked && <div className="absolute inset-0 grid place-items-center p-6 text-center text-sm text-[#746665]">Aucun aperçu n’est encore disponible.</div>}{isPartiallyMasked && <div className="absolute inset-y-0 right-0 w-1/2 bg-[#1b1c1c]/85 backdrop-blur-[2px]" aria-label="Contenu protégé" />}{isBlocked && <div className="absolute inset-0 flex flex-col items-center justify-center bg-white px-6 text-center"><span className="material-symbols-outlined text-5xl text-[#9e001f]">lock</span><h3 className="mt-4 text-xl font-bold text-[#2b2525]">Acheter pour lire tout le numéro</h3><p className="mt-2 max-w-sm text-sm leading-6 text-[#746665]">Les sept premières pages sont accessibles gratuitement. La lecture est protégée à partir de la page 8.</p><button type="button" onClick={onPurchase} className="mt-5 min-h-11 rounded-xl bg-[#9e001f] px-6 text-sm font-bold text-white">Choisir une version</button></div>}{!isBlocked && <span className="absolute bottom-3 left-3 rounded-full bg-[#1b1c1c]/75 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white">Page {page} / 7 gratuites</span>}</div></div>

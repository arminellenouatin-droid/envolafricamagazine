...
type WabGroup = { id: string; name: string; slug: string; logoUrl?: string; description?: string; privacy: "community" | "private"; status: string };
...
export default function ProfileClient() {
  ...
  async function createGroup(event: React.FormEvent) {
    ...
    try { const response = await fetch("/api/wab/groups", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: groupName, description: groupDescription, privacy: groupPrivacy }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setGroups((items) => [...items, data.group]); setGroupName(""); setGroupDescription(""); setGroupMessage("Groupe créé avec succès. Vous en êtes le propriétaire."); } catch (error) { setGroupMessage(error instanceof Error ? error.message : "Création impossible."); } finally { setGroupBusy(false); }
  }

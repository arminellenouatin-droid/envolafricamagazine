...
function ProductCard({ product }: { product: MarketplaceProduct & { isMagazine?: boolean; magazineId?: string; magazineNumero?: number } }) {
  ...
  return <article className="group overflow-hidden rounded-[22px] border border-[#eadfce] bg-white shadow-[0_12px_34px_rgba(74,48,18,0.06)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(74,48,18,0.13)]">
    ...
    <div className="p-4 lg:p-3">
      <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#a36300] lg:hidden">{product.category}</p><h3 className="mt-1 line-clamp-2 min-h-[44px] font-display text-[15px] font-extrabold leading-5 text-[#2a211a]">{product.title}</h3></div></div>
      <p className="mt-3 line-clamp-2 min-h-[38px] text-[12px] leading-5 text-[#725f4d] lg:hidden">{product.description}</p>
      <div className="mt-4 flex items-center justify-between gap-2"><span className="font-display text-[17px] font-black text-[#9e001f]">{formatXof(product.priceXof)}</span><span className="text-[11px] text-[#806c58] lg:hidden">{countryLabels[product.country] || product.country} · {product.city}</span></div>
      <div className="mt-3 flex items-center justify-between border-t border-[#f0e7dc] pt-3"><span className="max-w-[170px] truncate text-[11px] font-bold text-[#5d4a39] lg:hidden">{product.supplier}</span>{product.installment ? <span className="rounded-full bg-[#f5eee4] px-2 py-1 text-[10px] font-bold text-[#765326]">Paiement échelonné · jusqu’à {product.months} mois</span> : <span className="text-[10px] text-[#8d7b69]">Paiement comptant</span>}</div>
...
export default function MarketplaceClient() {
  ...
  return <div className="min-h-screen bg-[#fcf9f8] pb-24 text-[#2a211a]">
    ...
    <section id="catalogue" className="mx-auto max-w-[1280px] px-5 py-10 md:px-10 lg:px-16"><div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="font-display text-[11px] font-black uppercase tracking-[0.2em] text-[#a36300]">Le catalogue</p><h2 className="mt-2 font-display text-3xl font-black tracking-[-0.04em]">Trouvez votre prochaine opportunité</h2><p className="mt-2 text-sm text-[#806c58]">Les produits boostés apparaissent en premier, sans remplacer la pertinence de votre recherche.</p></div><div className="flex rounded-full border border-[#eadfce] bg-white p-1 shadow-sm"><button type="button" onClick={() => setViewMode("products")} className={`rounded-full px-4 py-2 text-[11px] font-black transition ${viewMode === "products" ? "bg-[#9e001f] text-white" : "text-[#725f4d] hover:bg-[#f8f3ed]"}`}>Produits</button><button type="button" onClick={() => setViewMode("vendors")} className={`rounded-full px-4 py-2 text-[11px] font-black transition ${viewMode === "vendors" ? "bg-[#9e001f] text-white" : "text-[#725f4d] hover:bg-[#f8f3ed]"}`}>Vendeurs</button></div></div>
      <div className="mt-7 rounded-[22px] border border-[#eadfce] bg-white p-3 md:p-4"><div className="grid gap-3 md:grid-cols-[1.6fr_1fr_1fr]"><div className="flex h-12 items-center gap-2 rounded-full bg-[#f8f3ed] px-4"><label className="grid h-9 w-9 shrink-0 cursor-pointer place-items-center rounded-full text-[#a36300] transition hover:bg-[#fff8ed]" title="Rechercher par image" aria-label="Rechercher par image"><span className="material-symbols-outlined text-[20px]">image_search</span><input type="file" accept="image/*" className="sr-only" disabled={imageSearchLoading} onChange={(event) => void handleImageSearch(event.target.files?.[0])} /></label><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un produit, une entreprise..." className="min-w-0 flex-1 bg-transparent text-[13px] outline-none" /><span className="material-symbols-outlined text-[20px] text-[#a36300]">search</span></div><select value={category} onChange={(event) => setCategory(event.target.value)} className="h-12 rounded-full bg-[#f8f3ed] px-4 text-[12px] font-bold text-[#5d4a39] outline-none">{marketplaceCategories.map((item) => <option key={item}>{item}</option>)}</select><select value={country} onChange={(event) => setCountry(event.target.value)} className="h-12 rounded-full bg-[#f8f3ed] px-4 text-[12px] font-bold text-[#5d4a39] outline-none">{countryOptions.map((item) => <option key={item.code} value={item.code}>{item.label}</option>)}</select></div><div className="mt-3 flex flex-wrap items-center gap-3 border-t border-[#f0e7dc] pt-3">{imageSearchLoading && <span className="text-[11px] font-semibold text-[#806c58]">Analyse de l’image en cours…</span>}{imageSearchPreview && <img src={imageSearchPreview} alt="Image utilisée pour la recherche" className="h-9 w-9 rounded-lg border border-[#eadfce] object-cover" />}{imageSearchMessage && <span className="text-[11px] font-semibold text-[#806c58]">{imageSearchMessage}</span>}</div></div>

...
type ManagedLandingItem = { id: string; title: string; description?: string; mediaUrl?: string; mediaType?: "image" | "video"; href?: string; date?: string; location?: string };
...
function LandingManagedContent({ blockKey, title, description, items, onChange }: { blockKey: string; title: string; description: string; items: ManagedLandingItem[]; onChange: (items: ManagedLandingItem[]) => void }) {
  ...
  const save = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!draft.title.trim()) return; const item = { ...draft, id: editingId || `${blockKey}-${Date.now()}`, title: draft.title.trim(), description: draft.description?.trim() || "", mediaUrl: draft.mediaUrl?.trim() || "", href: draft.href?.trim() || "", date: draft.date?.trim() || "", location: draft.location?.trim() || "" }; onChange(editingId ? items.map((entry) => entry.id === editingId ? item : entry) : [item, ...items]); reset(); };
...
export default function AdminDashboardClient({ user, stats, db }: { user: any, stats: any, db: any }) {
  ...
  const [landingSubTab, setLandingSubTab] = useState<"videos" | "contenus_sponsorises" | "formations_certifiees" | "recrutement">("videos");
  ...
  const fetchMagazineLanding = async () => { const res = await fetch("/api/admin/magazine-landing", { credentials: "include" }); const data = await readApiResponse(res); if (res.ok && Array.isArray(data.blocks)) setLandingBlocks(data.blocks); else if (!res.ok) setMessage(`Erreur Landing Magazine : ${data.error || `chargement impossible (${res.status})`}`); };

...
export async function GET() {
  ...
  if (supabase) {
    ...
    let storyQuery = supabase.from("wab_stories").select("id,user_id,author,media_url,mime_type,caption,views,likes,created_at,expires_at,visibility").eq("moderation_status", "published").gt("expires_at", new Date().toISOString()).order("created_at", { ascending: false });
  ...
  const stories = db.stories.filter((story) => story.moderationStatus === "published" && Date.parse(story.expiresAt) > Date.now() && (!user || premium || followedUserIds.includes(story.authorUserId ?? ""))).sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt));
...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => null) as { mediaUrl?: unknown; mimeType?: unknown; caption?: unknown; durationSeconds?: unknown } | null;
  ...
  if (mimeType.startsWith("video/") && (durationSeconds === undefined || !Number.isFinite(durationSeconds) || durationSeconds > 30.5)) return NextResponse.json({ error: "Une vidéo de Story ne peut pas dépasser 30 secondes." }, { status: 400 });
  const now = new Date(); const expiresAt = new Date(now.getTime() + 24 * 60 * 60 * 1000).toISOString(); const supabase = getSupabaseAdmin();
  if (supabase) { const premium = await isWabPremiumUser(user.id); const { data, error } = await supabase.from("wab_stories").insert({ user_id: user.id, author: `${user.prenom} ${user.nom}`, media_url: body.mediaUrl, mime_type: mimeType, caption: typeof body.caption === "string" ? body.caption.slice(0, 300) : "", created_at: now.toISOString(), expires_at: expiresAt, moderation_status: "published", visibility: premium ? "public" : "community" }).select("id,user_id,author,media_url,mime_type,caption,views,likes").single(); if (error || !data) return NextResponse.json({ error: "Impossible d’enregistrer la Story dans Supabase." }, { status: 503 }); return NextResponse.json({ story: { id: data.id, author: data.author, authorUserId: data.user_id, mediaUrl: data.media_url, mimeType: data.mime_type, caption: data.caption, views: data.views, likes: data.likes } }, { status: 201 }); }
  if (isProductionRuntime()) return NextResponse.json({ error: "Le stockage des Stories n’est pas configuré." }, { status: 503 });
  const story = { id: crypto.randomUUID(), author: `${user.prenom} ${user.nom}`, authorUserId: user.id, mediaUrl: body.mediaUrl, mimeType, caption: typeof body.caption === "string" ? body.caption.slice(0, 300) : "", createdAt: now.toISOString(), expiresAt, views: 0, likes: 0, moderationStatus: "published" as const };

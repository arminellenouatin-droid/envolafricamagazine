...
export default function AvantPremiereCarousel({ articles }: { articles: any[] }) {
  ...
  return <div className="col-span-12 group relative aspect-square w-full max-w-[1000px] overflow-hidden rounded-lg">
    <div className="relative h-full w-full overflow-hidden">
      {displayArticles.map((article: any, index: number) => {
        ...
        return <Link key={article.id} href={`/article/${article.slug}`} className={`absolute inset-0 h-full w-full transition-all duration-700 ease-in-out ${index === current ? "z-10 translate-x-0 opacity-100" : index < current ? "-translate-x-full opacity-0" : "translate-x-full opacity-0"}`}>
          ...
          <div className="absolute bottom-0 z-20 w-full p-6 text-white md:p-8">
            <div className="mb-3 flex flex-wrap gap-2">{categories.map((category: string) => <span key={category} className="inline-block rounded-full bg-[#9e001f] px-3 py-1 text-[10px] font-bold uppercase tracking-wider">{category}</span>)}</div>
            <h2 className="mb-2 line-clamp-3 text-[22px] font-bold leading-tight md:text-[28px]" style={{ fontFamily: "Montserrat" }}>{article.title}</h2>
            <p className="mb-3 hidden line-clamp-2 text-[14px] opacity-90 md:block" style={{ fontFamily: "Source Serif 4" }}>{article.summary}</p>
            <div className="flex items-center gap-2 text-[11px] opacity-80"><img src={article.authorProfilePhoto || "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=100"} alt="" className="h-6 w-6 rounded-full object-cover"/><span>{article.author}</span><span>•</span><span>{article.publishedAt ? new Date(article.publishedAt).toLocaleDateString("fr-FR") : "À la une"}</span></div>
          </div>
        </Link>;
      })}
    </div>

  ...
  return (
    <div className="bg-[#fcf9f8] min-h-screen pb-20">
      <div className="max-w-[1280px] mx-auto px-5 md:px-[64px] py-8">
        <div className="flex items-center gap-2 text-[11px] uppercase tracking-wider text-[#5c403f]"><Link href="/financement" className="hover:text-[#9e001f]">Crowdfunding</Link><span>ÔÇ║</span><span className="text-black font-bold">{projet.nom}</span></div>
        ...
        <div className="mt-6 grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            ...
            <div className="flex items-center gap-2 mt-3 text-[11px]"><span className="bg-[#ffdad8] text-[#9e001f] px-2 py-1 rounded-full font-bold uppercase">{projet.secteur}</span><span className="bg-[#f0eded] px-2 py-1 rounded-full">{projet.pays} ÔÇó {projet.niveauRisque} risque</span><span className="bg-green-100 text-green-700 px-2 py-1 rounded-full">{projet.statut}</span></div>
            <p className="text-[15px] leading-7 mt-4 text-[#1b1c1c]">{projet.description}</p>
            
            <div className="mt-8">
              <h3 className="font-bold text-[16px]">Vid├®os d'explication</h3>
              <div className="mt-3 aspect-video bg-[#1b1c1c] rounded-xl flex items-center justify-center text-white">Vid├®o pr├®sentation (├á uploader par porteur)</div>
            </div>

            <div className="mt-8 border-t border-[#e5bdbb]/30 pt-8">
              <h3 className="font-bold">Documents justificatifs</h3>
              <div className="mt-3 flex gap-2"><span className="px-3 py-1.5 rounded-full bg-[#f6f3f2] border text-[11px]">­ƒôä Carte d'identit├®</span><span className="px-3 py-1.5 rounded-full bg-[#f6f3f2] border text-[11px]">­ƒôä Registre commerce</span><span className="px-3 py-1.5 rounded-full bg-[#f6f3f2] border text-[11px]">­ƒôä Business plan PDF</span></div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-[16px] border border-[#e5bdbb] p-6 sticky top-24">
              ...
              <div className="mt-6 border-t border-[#e5bdbb]/30 pt-6">
                <h4 className="font-bold text-[14px]">3 fa├ºons d'aider</h4>
                <div className="mt-3 grid grid-cols-3 gap-2">
                  ...
                  <button onClick={()=>setTab("pret")} className={`h-10 rounded-full text-[11px] font-bold border ${tab==="pret"?"bg-[#9e001f] text-white border-[#9e001f]":"bg-white border-[#e5bdbb]"}`}>Pr├¬t</button>
                </div>

                {tab==="don" && (
                  <div className="mt-4">
                    <p className="text-[12px] text-[#5c403f]">Montant libre ou propos├®. Aucun retour financier. Badge Soutien sur profil.</p>
                    ...
                    <button onClick={() => startContribution("don", montant)} disabled={paying} className="mt-4 w-full h-11 rounded-full bg-[#9e001f] text-white font-bold text-[13px] disabled:opacity-50">{paying ? "Redirection Moneroo..." : `Faire un don de ${montant.toLocaleString()} F ÔåÆ`}</button>
                  </div>
                )}

                {tab==="prise_part" && (
                  <div className="mt-4">
                    <p className="text-[12px] text-[#5c403f]">Achetez % entreprise. Valorisation auto: montant collect├® / % vendu = {valorisation.toLocaleString()} F. Contrat PDF auto g├®n├®r├®.</p>
                    ...
                    <div className="mt-3"><label className="text-[11px] font-bold">% souhait├®</label><input type="range" min="0.1" max="10" step="0.1" value={pourcentage} onChange={(e) => setPourcentage(Number(e.target.value))} className="w-full mt-1" /><div className="text-[11px] text-[#5c403f]">{pourcentage}% = {Math.round(valorisation * pourcentage / 100).toLocaleString()} F</div></div>
                    <button onClick={() => startContribution("prise_part", Math.round(valorisation * pourcentage / 100), pourcentage)} disabled={paying} className="mt-4 w-full h-11 rounded-full bg-[#9e001f] text-white font-bold text-[13px] disabled:opacity-50">{paying ? "Redirection Moneroo..." : `Acheter ${pourcentage}% pour ${Math.round(valorisation * pourcentage / 100).toLocaleString()} F ÔåÆ Contrat PDF auto`}</button>
                  </div>
                )}

                {tab==="pret" && (
                  <div className="mt-4">
                    <p className="text-[12px] text-[#5c403f]">Pr├¬tez avec taux {projet.tauxInteret}% fix├® par porteur. Calendrier remboursement auto: fr├®quence mensuelle, capital+int├®r├¬ts, date paiement. Suivi retards + messages auto.</p>
                    <div className="mt-3 bg-[#f6f3f2] rounded-lg p-3 text-[12px]">
                      <div className="flex justify-between"><span>Taux</span><span className="font-bold">{projet.tauxInteret}%/an</span></div>
                      <div className="flex justify-between mt-1"><span>Dur├®e</span><span>{projet.dureeJours} jours</span></div>
                      <div className="mt-2 text-[10px]">Calendrier: chaque mois, part capital + int├®r├¬ts, date paiement auto</div>
                    </div>
                    <input type="number" value={montant} onChange={e=>setMontant(parseInt(e.target.value)||0)} placeholder="Montant ├á pr├¬ter" className="mt-3 w-full h-11 rounded-full border bg-[#f6f3f2] px-4 text-[14px]" />
                    <button onClick={() => startContribution("pret", montant)} disabled={paying} className="mt-3 w-full h-11 rounded-full bg-[#9e001f] text-white font-bold text-[13px] disabled:opacity-50">{paying ? "Redirection Moneroo..." : `Pr├¬ter ${montant.toLocaleString()} F ├á ${projet.tauxInteret}% ÔåÆ`}</button>
                  </div>
                )}
              </div>
